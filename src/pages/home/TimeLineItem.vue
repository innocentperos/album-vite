<template>
  <div class="w-full p-4 flex flex-col dark:bg-gradient-to-br bg-white dark:from-indigo-600 dark:to-indigo-800 dark:via-indigo-700 rounded-lg dark:text-white">
    <div class="flex space-x-2">
      <Avatar history xsmall class="shrink-0" />
    ...
    <div class="flex flex-col space-y-2 mt-4">
      <span class="text-sm tracking-wider">added {{count}} pictures to album</span>
    ...
    <div class="pt-2 flex items-center ">
      ...
      <div class="p-1 flex items-center">
        ...
        <IconButton text loading icon="bx-chat" class="text-2xl font-light" />
      </div>
...
<script setup>
  ...
  const gridSize = computed(function() {
    ...
    if (c % 4 == 0) return 'grid-cols-4'

    return 'grid-cols-' + Math.min(Math.round(Math.sqrt(c)) + 1, 4)

    if (c < 4) return 'grid-cols-' + (c + 1)
